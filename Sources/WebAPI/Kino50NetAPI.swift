...
open class Kino50NetAPI: HttpService {
  public static let SiteUrl = "https://kino50.net"
  let UserAgent = "Kino50Net User Agent"
...
//  }
  ...
  public func available() throws -> Bool {
    if let document = try getDocument(Kino50NetAPI.SiteUrl) {
      return try document.select("div[id=main-containe]").size() > 0
    }
    else {
      return false
    }
