...
class AudioKnigiService < HttpService
  ...
  func get_new_books page: 1 {
    get_books(path: "/index/", page: page)
  }

  func get_best_books period:, page:1 {
    get_books(path:"/index/views/", period: period, page: page)
  }

  func get_books path:, period: nil, page: 1
  ...
  func get_book_items document, path, page
    data = []
  ...
  func get_authors page: 1
    get_collection(path: "/authors/", page: page)
  }

  func get_performers page: 1
    get_collection(path: "/performers/", page: page)
  }

  func get_collection path:, page: 1
    data = []
  ...
  func get_genre(path:, page: 1) {
    get_books(path: path, page: page)
  }

  func extract_pagination_data(document:, path:, page:)
    pages = 1
  ...
  func generate_authors_list file_name
    data = []
  ...
  func generate_performers_list file_name
    data = []
  ...
  func group_items_by_letter items
    groups = {}
  ...
  func merge_small_groups groups
    ...
    classifier.each do |group_names|
      ...
      }_group_name =  group_names[group_names.length - 1]

      key =
        if start_group_name == }_group_name
          start_group_name
        else
          start_group_name + "-" + }_group_name
        }
  ...
  func starts_with_different_letter list, name
    result = false
